"""Visualize a network's extrapolation on a real surface, with an error map.

Like viz_prediction.py, but the third panel is an ERROR MAP instead of the
ground truth. For a trained run + a surface + a mask, renders three linked
3-D views:
  ...
  3. error map     — the predicted surface coloured by the per-node difference
                     from the ground truth (prediction - truth, in metres).
The first two panels are coloured by region (blue = K / known input,
orange = U / extrapolated), so it is obvious which part was given and which
was predicted. The third panel reuses the predicted geometry but recolours it
by signed error on a diverging map (or |error| with --abs-error): the observed
K region is ~0 by construction, so the colour concentrates where the model
actually had to extrapolate.
...
Usage:
    python scripts/viz_prediction_with_error.py outputs/tensorboard/run_XXXX --surface TestHorizon4
    python scripts/viz_prediction_with_error.py outputs/tensorboard/run_XXXX --split test_ood --index 0
    python scripts/viz_prediction_with_error.py ... --abs-error      # |error| instead of signed
    python scripts/viz_prediction_with_error.py ... --show-edges     # draw triangle edges
    python scripts/viz_prediction_with_error.py ... --out fig.png    # off-screen PNG instead of a window
    python scripts/viz_prediction_with_error.py ... --list           # list surfaces in the split and exit
"""
...
SIGNED_CMAP = "coolwarm"  # diverging: blue = under-predict, red = over-predict
ABS_CMAP = "inferno"      # sequential: magnitude of the error
...
def main() -> None:
    ...
    p.add_argument("--abs-error", action="store_true",
                   help="colour the error panel by |error| (sequential) instead of signed error (diverging)")
    args = p.parse_args()
    ...
    # per-node error (prediction - truth) in centred metres; scale the colour
    # range to the extrapolated region, where the error actually lives.
    error = (z_nn - z_true).detach().cpu().numpy()
    unknown = (~mask).cpu().numpy()
    emax = float(np.abs(error[unknown]).max()) if unknown.any() else float(np.abs(error).max())
    emax = max(emax, 1e-6)
    if args.abs_error:
        err_scalars, err_cmap, err_clim = np.abs(error), ABS_CMAP, (0.0, emax)
        bar_title = "|error| (m)"
    else:
        err_scalars, err_cmap, err_clim = error, SIGNED_CMAP, (-emax, emax)
        bar_title = "pred - truth (m)"

    off = args.out is not None
    pl = pv.Plotter(shape=(1, 3), window_size=(1800, 650), off_screen=off)

    # panels 1 & 2: region-coloured (blue = K input, orange = U extrapolated)
    region_panels = [("input  (blue=K, orange=U)", z_input),
                     (f"network extrapolation  (U-RMSE {rmse:.0f} m)", z_nn)]
    for col, (title, z) in enumerate(region_panels):
        pl.subplot(0, col)
    ...
    # panel 3: error map on the predicted surface
    pl.subplot(0, 2)
    pl.add_text(f"error map  (U-RMSE {rmse:.0f} m)", font_size=10)
    me = pv_mesh(xy, z_nn, surface.F)
    me["error"] = err_scalars
    pl.add_mesh(me, scalars="error", cmap=err_cmap, clim=err_clim,
                show_edges=args.show_edges,
                scalar_bar_args={"title": bar_title, "fmt": "%.0f"})
